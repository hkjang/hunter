...
for(const [tab,name] of [['SSO · 로그인','oidc'],['AI 분석','ai'],['검토 · 승인','workflow'],['보안 · 세션','security'],['역할 · 권한','roles']]){await page.getByRole('button',{name:tab,exact:true}).click();await shot('admin-settings-'+name);}
...
await page.goto(base+'/dashboard');await settle();await page.locator('.profile-trigger').click();await page.getByText(/서비스 버전 v1\.0\.0/).waitFor();await shot('profile-menu');await page.keyboard.press('Escape');
const mobile=[];

...
const entries=[['user-guide','사용자 가이드','자동 SSO·로컬 복구, 조치함·진단·에이전트 재개·한글 보고서·API 키 사용 절차'],['admin-guide','관리자 가이드','폐쇄망 설치, OIDC 자동 진입·격리 방문 추적, 선택 모델·진단·자동화·권한·백업 절차']];
const routes=[
['login','로그인','로컬 계정과 SSO 로그인, 서비스 버전'],
['sso-local-login','SSO 확인 후 로컬 로그인','v1.9 합성 OIDC: 기존 세션 확인 후 명시 로그인 복구'],
['admin-tracking','관리자 방문 추적','v1.9 공개 코드·허용 원점·암호화 revision 설정'],
['tracking-preview','방문 추적 격리 미리보기','v1.9 로컬 수집기용 합성 이벤트와 격리 실행 안내'],

...
사내 메일 표준(MAIL-STANDARD)에 맞춘 **메일 알림**은 서비스 설정의 `mail.*` 키 하나로 사내 SMTP 릴레이(포트 25·인증 없음·TLS 없음이 기본, 인증과 암호화는 선택)를 가리키고, 사람이 실제로 기다리는 다섯 가지 — 진단 검토 요청·검토 결과·진단 실패·에이전트 입력 대기·에이전트 실행 중단 — 만 보냅니다. 메일은 배경에서 보내므로 릴레이가 죽어 있어도 요청은 정상으로 끝나고, 시도마다 성공·실패가 기록되며(본문은 기록하지 않음), 자기가 한 일은 자기에게 보내지 않고 한 번에 생긴 여러 건은 한 통으로 묶습니다. 기본은 꺼짐이며 관리 화면에 **시험 발송**과 발송 기록이 있습니다. [관리자 가이드 §5.8](docs/guides/admin-guide.md#58-사내-smtp-릴레이-메일-알림)을 참고하세요.

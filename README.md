...
v1.16.0은 **목록 주소 복사**가 저장한 보기의 보관 한도에 잘리지 않고 지금 보고 있는 목록을 그대로 가리키도록 고칩니다. 주소를 만드는 `listSharePath` 는 저장한 보기와 같은 함수를 썼는데, 그 함수의 500자 자르기는 브라우저 저장소 한도용이었습니다. 그래서 검색어나 필터가 500자를 넘으면 복사한 주소가 보낸 사람이 보던 것과 다른 결과를 열었습니다. 이제 `savedListQuery` 가 자르기 여부를 명시적으로 받고 주소 복사만 자르지 않으므로, 복사한 주소는 검색·필터 텍스트를 온전히 담습니다. 저장한 보기의 500자 한도와 허용 조건만 보관하는 규칙은 그대로입니다. [릴리즈 노트](docs/release-v1.16.0.md)를 참고하세요.
...
~~~sh
docker load -i hunter-v1.16.0.tar.gz
cp .env.example .env
chmod 600 .env
...
| 항목 | 형식 | v1.16.0 예시 |
| --- | --- | --- |
| Docker 이미지 | hunter:v버전 | hunter:v1.16.0 |
| 유일한 첨부 자산 | hunter-v버전.tar.gz | hunter-v1.16.0.tar.gz |

~~~sh
bash scripts/release.sh 1.16.0
~~~

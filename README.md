...
v1.6.0은 발견 건·조치 기한·진단 결과·승인 대기를 이메일과 조직의 메시지 API로 연결합니다. 관리자는 고정 수신자·필터·템플릿을 설정하고 전달 이력과 제한된 재시도를 확인합니다. SMTP는 TLS·필수 STARTTLS, HTTP는 본문 매핑·성공 판정·NCP 서명 등을 지원합니다. **게이트웨이 접수는 최종 수신·읽음 확인과 다릅니다.** [알림 채널 공식 근거](docs/research-notifications.md)와 [관리자 운영 가이드](docs/guides/admin-guide.md#136-알림센터와-발송-채널)를 참고하세요.
...
~~~sh
docker load -i hunter-v1.6.0.tar.gz
cp .env.example .env
chmod 600 .env
...
| 항목 | 형식 | v1.6.0 예시 |
| --- | --- | --- |
| Docker 이미지 | hunter:v버전 | hunter:v1.6.0 |
| 유일한 첨부 자산 | hunter-v버전.tar.gz | hunter-v1.6.0.tar.gz |

~~~sh
bash scripts/release.sh 1.6.0
~~~
